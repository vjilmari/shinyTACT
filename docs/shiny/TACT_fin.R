TACT_fin <- function(r=NULL, x=NULL, y=NULL, distribution = c("normal", "uniform", "skewed"), n = 10^6,
  ...
  if(plot){
    ...
    if(abs(diff(cutoffsx)) > 0.001 & abs(diff(cutoffsy)) > 0.001)
      mtext(paste(round(sum(diag(t(apply(crosstabs, 2, rev)))) / sum(crosstabs),3)*100,
                  ...
                  TACT_languages[TACT_languages$language==language,"match_2"],sep=""), side = 3, line=-5.0, adj=0.04, cex=font_size)
